...
// TestRunFlushesUnconsumedSteersOnCancel proves that a steer that is still
// queued when the turn is cancelled survives into the session (history + next
// turn's context) and emits its Steer event, instead of being silently
// dropped (#6238: queued guidance vanished from both the model and the
// transcript).
...
// blocks (for Chinese text even in auto mode) ahead of the steer prefix.
// SteerText must skip the wrapping and return the user's exact original text,
// or replay degrades the steer into a plain user message.
func TestSteerTextSurvivesTurnPreferenceWrapping(t *testing.T) {
	...
	cases := []struct {
		...
		{"exact text preserved", explicit, "  spaced\ttext  "},
	}
