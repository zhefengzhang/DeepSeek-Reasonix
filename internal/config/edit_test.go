...
func TestNormalizeEffortDeepSeek(t *testing.T) {
	...
	for in, want := range map[string]string{"auto": "", "disabled": "disabled", "high": "high", "max": "max", "off": "disabled", "low": "high", "medium": "high", "xhigh": "max"} {
	...
	if _, err := NormalizeEffort(e, "turbo"); err == nil {
		t.Fatal("DeepSeek /effort must reject invalid level")
	}
}
