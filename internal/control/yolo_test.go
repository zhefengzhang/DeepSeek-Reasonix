...
func TestRequestApprovalHonorsAutoApproveTools(t *testing.T) {
	...
	go func() {
		allow, _, _, err := c.requestApproval(context.Background(), "multi_edit", "/tmp/file", nil)
		if err != nil {
...
func TestMemoryApprovalIgnoresAutoApproveTools(t *testing.T) {
	...
	go func() {
		allow, _, _, err := c.requestApproval(context.Background(), "remember", "", nil)
		if err != nil {
			errs <- err
			return
		}
...
func TestToolApprovalModeAutoDoesNotDrainPendingExplicitAsk(t *testing.T) {
	...
	errs := make(chan error, 1)
	go func() {
		allow, _, _, err := c.requestApproval(context.Background(), "bash", "git commit -m x", nil)
		if err != nil {
			errs <- err
			return
		}
...
func TestToolApprovalModeYoloBypassesApprovalPrompts(t *testing.T) {
	...
	allow, remember, _, err := c.requestApproval(context.Background(), "bash", "go test ./...", nil)
...
func TestSetAutoApproveToolsDoesNotDrainPendingMemoryApproval(t *testing.T) {
	...
	errs := make(chan error, 1)
	go func() {
		allow, _, _, err := c.requestApproval(context.Background(), "forget", "", nil)
		if err != nil {
			errs <- err
			return
		}
...
func TestSetModeYoloDrainsPendingApproval(t *testing.T) {
	...
	go func() {
		allow, _, _, _ := c.requestApproval(context.Background(), "multi_edit", "/tmp/file", nil)
		done <- allow
	}()
