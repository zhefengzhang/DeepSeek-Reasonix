...
func (g grepTool) Description() string {
	if g.rg != "" {
		return "Search for a regular expression in a file, or recursively under a directory — ripgrep-backed, so it honors .gitignore. Returns matching lines as path:line:text, capped at 200 matches. PREFER `understand_search` or `codegraph_explore` for structural source-code queries — grep is a fallback for exact text/regex matching."
	}
	return "Search for a regular expression in a file, or recursively under a directory (skips hidden files and files matched by .gitignore). Returns matching lines as path:line:text, capped at 200 matches. PREFER `understand_search` or `codegraph_explore` for structural source-code queries — grep is a fallback for exact text/regex matching."
}
